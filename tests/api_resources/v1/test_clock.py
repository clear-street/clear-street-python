...
from clear_street.types.v1 import ClockGetClockResponse
...
class TestClock:
    ...
    def test_method_get_clock(self, client: ClearStreet) -> None:
        clock = client.v1.clock.get_clock()
    ...
    def test_raw_response_get_clock(self, client: ClearStreet) -> None:
        response = client.v1.clock.with_raw_response.get_clock()
    ...
    def test_streaming_response_get_clock(self, client: ClearStreet) -> None:
        with client.v1.clock.with_streaming_response.get_clock() as response:
...
class TestAsyncClock:
    ...
    async def test_method_get_clock(self, async_client: AsyncClearStreet) -> None:
        clock = await async_client.v1.clock.get_clock()
        assert_matches_type(ClockGetClockResponse, clock, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_raw_response_get_clock(self, async_client: AsyncClearStreet) -> None:
        response = await async_client.v1.clock.with_raw_response.get_clock()
    ...
    async def test_streaming_response_get_clock(self, async_client: AsyncClearStreet) -> None:
        async with async_client.v1.clock.with_streaming_response.get_clock() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
