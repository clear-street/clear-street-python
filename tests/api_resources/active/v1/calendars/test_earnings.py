...
class TestEarnings:
    ...
    def test_streaming_response_get_earnings_calendar(self, client: ClearStreet) -> None:
        with client.active.v1.calendars.earnings.with_streaming_response.get_earnings_calendar(
            from_date="from_date",
            to_date="to_date",
        ) as response:
...
class TestAsyncEarnings:
    ...
    async def test_streaming_response_get_earnings_calendar(self, async_client: AsyncClearStreet) -> None:
        async with async_client.active.v1.calendars.earnings.with_streaming_response.get_earnings_calendar(
            from_date="from_date",
            to_date="to_date",
        ) as response:
            assert not response.is_closed
